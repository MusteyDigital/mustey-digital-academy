                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="border-b border-slate-200">
                                ...
                                <th class="text-left py-3 px-4 text-xs uppercase tracking-wide text-slate-500 font-medium">Lesson</th>
                                <th class="text-left py-3 px-4 text-xs uppercase tracking-wide text-slate-500 font-medium">Instructor</th>
                                <th class="text-left py-3 px-4 text-xs uppercase tracking-wide text-slate-500 font-medium">Completed At</th>
                            </tr>
                        </thead>
                            ...
                            @forelse($records as $r)
                                <tr class="border-b border-slate-100 last:border-b-0 hover:bg-slate-50 transition">
                                    ...
                                    <td class="py-3 px-4">
                                        ...
                                        @if($course && $course->id)
                                            <a class="text-blue-600 font-medium hover:text-blue-700 transition" href="{{ route('courses.show', $course->id) }}">
                                                {{ $course->title }}
                                            </a>
                                        @else
                                    ...
                                    <td class="py-3 px-4 text-slate-500">
                                        {{ $r->completed_at ? $r->completed_at->format('M j, Y g:ia') : '—' }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
